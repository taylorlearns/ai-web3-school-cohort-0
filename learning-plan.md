# Taylor 的个人学习计划

> AI × Web3 School — 四阶段学习路线

学习路线总览：AI 基础 → Web3 基础 → AI × Web3 Bridge → 前沿探索

## Phase 1：AI 基础（预计 2 周）

- [x] [LLM](https://aiweb3.school/zh/handbook/ai/llm/) — 大语言模型（2026-05-18 ✅）
- [x] [Prompt](https://aiweb3.school/zh/handbook/ai/prompt/) — 提示词（2026-05-19 ✅）
- [x] [Context](https://aiweb3.school/zh/handbook/ai/context/) — 上下文（2026-05-19 ✅）
- [ ] [RAG](https://aiweb3.school/zh/handbook/ai/rag/) — 检索增强生成
- [ ] [Agent](https://aiweb3.school/zh/handbook/ai/agent/) — 智能体
- [ ] [Frameworks](https://aiweb3.school/zh/handbook/ai/frameworks/) — 框架
- [ ] [MCP](https://aiweb3.school/zh/handbook/ai/mcp/) — 模型上下文协议
- [ ] [Evaluation](https://aiweb3.school/zh/handbook/ai/evaluation/) — 评估
- [ ] [Fine-tuning](https://aiweb3.school/zh/handbook/ai/fine-tuning/) — 微调
- [ ] [Inference](https://aiweb3.school/zh/handbook/ai/inference/) — 推理服务

## Phase 2：Web3 基础（预计 2 周）

- [ ] [Network](https://aiweb3.school/zh/handbook/web3/network/) — 区块链网络
- [ ] [Cryptography](https://aiweb3.school/zh/handbook/web3/cryptography/) — 密码学
- [ ] [Wallet](https://aiweb3.school/zh/handbook/web3/wallet/) — 钱包
- [ ] [Smart Contract](https://aiweb3.school/zh/handbook/web3/smart-contract/) — 智能合约
- [ ] [Account Abstraction](https://aiweb3.school/zh/handbook/web3/account-abstraction/) — 账户抽象
- [ ] [DeFi](https://aiweb3.school/zh/handbook/web3/defi/) — 开放金融
- [ ] [Oracle](https://aiweb3.school/zh/handbook/web3/oracle/) — 预言机
- [ ] [Indexing](https://aiweb3.school/zh/handbook/web3/indexing/) — 索引
- [ ] [Security](https://aiweb3.school/zh/handbook/web3/security/) — 安全

## Phase 3：AI × Web3 Bridge（预计 3 周）

- [ ] [Chain-aware Context](https://aiweb3.school/zh/handbook/bridge/chain-aware-context/)
- [ ] [Web3 Tool Use](https://aiweb3.school/zh/handbook/bridge/web3-tool-use/)
- [ ] [Agent Workflow](https://aiweb3.school/zh/handbook/bridge/agent-workflow/)
- [ ] [Agent Wallet](https://aiweb3.school/zh/handbook/bridge/agent-wallet/)
- [ ] [Machine Payment](https://aiweb3.school/zh/handbook/bridge/machine-payment/)
- [ ] [Settlement & Escrow](https://aiweb3.school/zh/handbook/bridge/settlement-and-escrow/)
- [ ] [Agent Identity](https://aiweb3.school/zh/handbook/bridge/agent-identity/)
- [ ] [Agent Trust & Reputation](https://aiweb3.school/zh/handbook/bridge/agent-trust-and-reputation/)
- [ ] [Verifiable AI](https://aiweb3.school/zh/handbook/bridge/verifiable-ai/)
- [ ] [AI Security](https://aiweb3.school/zh/handbook/bridge/ai-security/)
- [ ] [AI Privacy](https://aiweb3.school/zh/handbook/bridge/ai-privacy/)
- [ ] [Governance AI](https://aiweb3.school/zh/handbook/bridge/governance-ai/)

## Phase 4：前沿探索（项目实战）

- [ ] Agentic Commerce — 智能体商业
- [ ] Wallet / Permission — 钱包与权限
- [ ] AI Security — AI 安全
- [ ] Governance — 治理
- [ ] Dev Tooling — 开发工具
- [ ] Open Track — 开放赛道
